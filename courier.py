...
active_timers = {}
# Ovoz bergan kuryerlar to'plami
break_votes_set = set()
break_task = None
...
async def lunch_break(message: Message, bot: Bot):
    ...
    if user_id in break_votes_set:
        await message.answer("✅ Siz allaqachon ovoz bergansiz. Boshqa kuryerlar ham bosishini kuting.")
        return
    ...
    await message.answer(
        ...
        "Barcha kuryerlar ovoz bersa, tushlik boshlanadi."
    )
    ...
    if break_votes_set >= all_ids:
        ...
        # Tushlik boshlash va bazada holatni yangilash
        for courier in couriers:
            try:
                await db.set_courier_break(courier['user_id'], True)
                await bot.send_message(
                    courier['user_id'],
...
async def break_timer(bot: Bot, courier_ids: list):
    ...
    # Tushlik tugadi va bazada holatni tiklash
    for uid in courier_ids:
        try:
            await db.set_courier_break(uid, False)
            await bot.send_message(
                uid,
