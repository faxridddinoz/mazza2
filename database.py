...
from keyboards import (
    ...
    cancel_keyboard, order_action_keyboard
)
...
async def my_orders(message: Message):
    ...
    if order['status'] == 'accepted' and order['accepted_at'] and order['delivery_minutes']:
        try:
            ...
            logger.error(f"Taymer hisoblashda xato: {e}")
...
async def get_location(message: Message, state: FSMContext, bot: Bot):
    ...
    if not cart:
        await state.clear()
        await message.answer("❌ Savat bo'sh, qaytadan boshlang.", reply_markup=user_main_keyboard())
        return
    ...
    # Savat ma'lumotlarini tayyorlaymiz
    items_list = []
    total = 0
    ...
    # Admin va kuryerlarga xabar matni
    order_text = (
    ...
    for admin_row in admins:
        try:
            await bot.send_message(
                admin_row['user_id'], order_text,
                reply_markup=order_action_keyboard(order_id),
                parse_mode="HTML"
            )
            notified.add(admin_row['user_id'])
        except Exception as e:
            logger.error(f"Admin {admin_row['user_id']} ga xabar ketmadi: {e}")

    for courier_row in couriers:
        if courier_row['user_id'] not in notified and not courier_row['is_on_break']:
            try:
                await bot.send_message(
                    courier_row['user_id'], order_text,
                    reply_markup=order_action_keyboard(order_id),
                    parse_mode="HTML"
                )
            except Exception as e:
                logger.error(f"Kuryer {courier_row['user_id']} ga xabar ketmadi: {e}")
